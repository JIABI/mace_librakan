...
# =========================================================================================
# Helper: call ctor with only supported kwargs (prevents "unexpected keyword argument")
# =========================================================================================
def _call_with_supported_kwargs(ctor: Callable, **kwargs):
    """
    ...
    # If **kwargs present, pass through
...
def _maybe_irreps(x: Union[o3.Irreps, str]) -> o3.Irreps:
    ...
    return o3.Irreps(x) if isinstance(x, str) else x


def _build_readout(
    readout_cls: Type[torch.nn.Module],
    hidden_irreps_out: o3.Irreps,
    mlp_irreps: o3.Irreps,
    gate: Optional[Callable],
    heads: int,
    cueq_config: Optional[Dict[str, Any]],
    oeq_config: Optional[Dict[str, Any]],
    readout_kwargs: Optional[Dict[str, Any]] = None,
) -> torch.nn.Module:
    readout_kwargs = readout_kwargs or {}
    return readout_cls(
        irreps_in=hidden_irreps_out,
        MLP_irreps=(heads * mlp_irreps).simplify(),
        gate=gate,
        irrep_out=o3.Irreps(f"{heads}x0e"),
        num_heads=heads,
...
class MACE(torch.nn.Module):
    ...
    def __init__(
        ...
        # ----- first interaction (safe filtered kwargs) -----
        inter = _call_with_supported_kwargs(
            ...
            if i == num_interactions - 2:
                self.readouts.append(
                    _build_readout(
                        ...
                        hidden_irreps_out=_maybe_irreps(hidden_irreps_out),
                        mlp_irreps=MLP_irreps,
                        gate=gate,
                        heads=len(heads),
                        cueq_config=cueq_config,
                        oeq_config=oeq_config,
                        readout_kwargs=self._readout_kwargs,
                    )
                )
    ...
    def forward(
        ...
        if hasattr(self, "pair_repulsion"):
            ...
            pair_energy = scatter_sum(src=pair_node_energy, index=data["batch"], dim=0, dim_size=num_graphs)
        else:
...
class AtomicDipolesMACE(torch.nn.Module):
    def __init__(
        ...
        edge_irreps: Optional[o3.Irreps] = None,
        interaction_mlp_factories: Optional[Dict[str, Optional[Callable[..., torch.nn.Module]]]] = None,
        readout_cls: Type[torch.nn.Module] = LinearDipoleReadoutBlock,
        readout_kwargs: Optional[Dict[str, Any]] = None,
    ):
        super().__init__()
        self.register_buffer("atomic_numbers", torch.tensor(atomic_numbers, dtype=torch.int64))
        ...
        self.readouts = torch.nn.ModuleList()
        self.readouts.append(readout_cls(hidden_irreps, dipole_only=True, **self._readout_kwargs))

        for i in range(num_interactions - 1):
            hidden_irreps_out = str(hidden_irreps[1]) if i == num_interactions - 2 else hidden_irreps
            inter = _call_with_supported_kwargs(
                interaction_cls,
                node_attrs_irreps=node_attr_irreps,
                node_feats_irreps=hidden_irreps,
...
class AtomicDielectricMACE(torch.nn.Module):
    def __init__(
        ...
        interaction_mlp_factories: Optional[Dict[str, Optional[Callable[..., torch.nn.Module]]]] = None,
        readout_cls: Type[torch.nn.Module] = LinearDipolePolarReadoutBlock,
        readout_kwargs: Optional[Dict[str, Any]] = None,
    ):
        super().__init__()
        self.register_buffer("atomic_numbers", torch.tensor(atomic_numbers, dtype=torch.int64))
        ...
        self.readouts = torch.nn.ModuleList()
        self.readouts.append(readout_cls(hidden_irreps, use_polarizability=True, **self._readout_kwargs))
        ...
        for i in range(num_interactions - 1):
            ...
            self.products.append(prod)
            if i == num_interactions - 2:
                if readout_cls is NonLinearDipolePolarReadoutBlock:
...
class EnergyDipolesMACE(torch.nn.Module):
    def __init__(
        ...
        self.readouts = torch.nn.ModuleList()
        self.readouts.append(readout_cls(hidden_irreps, dipole_only=False, **self._readout_kwargs))

        for i in range(num_interactions - 1):
            hidden_irreps_out = str(hidden_irreps[:2]) if i == num_interactions - 2 else hidden_irreps
            inter = _call_with_supported_kwargs(
                interaction_cls,
                node_attrs_irreps=node_attr_irreps,
                node_feats_irreps=hidden_irreps,
    ...
    def forward(
        ...
        for interaction, product, readout in zip(self.interactions, self.products, self.readouts):
            ...
            # keep node-wise energies for return
            node_energies_list.append(node_energies)
            energy = scatter_sum(src=node_energies, index=data["batch"], dim=0, dim_size=num_graphs)
            energies.append(energy)
